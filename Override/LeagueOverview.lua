...
function GetActiveResolutions(league, activeiPlayer)
	...
	for _, t in ipairs(league:GetActiveResolutions()) do
		...
		local canProposeRepeal = league:CanProposeRepeal(resolutionId, activeiPlayer);
		local text = league:GetResolutionName(resolutionType, resolutionId, proposerDecision, false);
		local toolTip = league:GetResolutionDetails(resolutionType, activeiPlayer, resolutionId, proposerDecision); 
...
function GetInactiveResolutions(league, activeiPlayer)
	...
	for _, t in ipairs(league:GetInactiveResolutions()) do
		...
		local canProposeEnact = league:CanProposeEnactAnyChoice(resolutionType, activeiPlayer);
		...
		local choices = league:GetChoicesForDecision(resolutionDecisionId, activeiPlayer);
		...
		for _, choiceId in ipairs(choices) do
			local choice = {
				...
				ToolTip = league:GetResolutionDetails(resolutionType, activeiPlayer, resolutionId, choiceId);
				Disabled = not league:CanProposeEnact(resolutionType, activeiPlayer, choiceId)
			}
...
function GetLeagueMembers(league)
	...
	local members = {};
	if(league ~= nil) then
		local hostiPlayer = league:GetHostMember();
		--for iPlayer = 0, GameDefines.MAX_MAJOR_CIVS - 1, 1 do
		for iPlayer = 0, 21, 1 do -- hardcoded in XML = hardcoded here...
...
function GetProposals(league, activeiPlayer)
	...
	local GetChoicesForResolution = function(resolutionType) 
		...
		if(decisionType ~= "RESOLUTION_DECISION_YES_OR_NO") then
			...
			local choices = league:GetChoicesForDecision(decisionId, activeiPlayer);
	...
	for i,v in ipairs(enactProposals) do
		...
		local proposal = {
			...
			ToolTip = league:GetResolutionDetails(v.Type, activeiPlayer, v.ID, choice);
			OnHold = false;
		};
	...
	for i,v in ipairs(enactProposalsOnHold) do
		...
		local proposal = {
			...
			ToolTip = "[COLOR_WARNING_TEXT]" .. Locale.Lookup("TXT_KEY_LEAGUE_OVERVIEW_PROPOSAL_ON_HOLD_TT") .. "[ENDCOLOR][NEWLINE][NEWLINE]" .. league:GetResolutionDetails(v.Type, activeiPlayer, v.ID, choice);
			OnHold = true;
		};
...
function GatherAdditionalChoiceInfoBasedOnDecisionType(decisionType, choice)
	if(decisionType == "RESOLUTION_DECISION_MAJOR_CIV_MEMBER" or decisionType == "RESOLUTION_DECISION_OTHER_MAJOR_CIV_MEMBER") then
		...
		choice.UseiPlayerForSubIcon = true;
		choice.iPlayer = choice.Id;
	end
...
function ViewCurrentProposals(model)
	...
	elseif(model.ViewMode == "Propose") then
		...
		ProposalController:Initialize{
			...
			ActiveiPlayer = model.ActivePlayer,
...
function ViewLeagueMembers(members)
	...
	for i, member in ipairs(members) do
		...
		CivIconHookup(member.iPlayer, subIconSize, subIcon, subIconBG, subIconShadow, true, true, subIconHighlight);
		...
		buttonControl:RegisterCallback( Mouse.eLClick, function() OnLeaderSelected(member.iPlayer); end );
...
function PopulateResolutionChoicePopup(proposerChoices, selectFunction)
	...
	for _, v in ipairs(proposerChoices) do
		...
		instance.SubIconFrame:SetHide(true);
		if(v.UseiPlayerForSubIcon) then
			
			CivIconHookup(v.iPlayer, 32, instance.SubIcon, instance.SubIconBG, instance.SubIconShadow, true, true);
...
function VoteController:Initialize(data)
	...
	self.ActiveiPlayer = data.ActiveiPlayer;
		...
		for _, entry in ipairs(controller.VoteEntries) do
			...
			if(entry.Votes ~= 0) then
				...
				if(entry.Direction == "Enact") then		
					Network.SendLeagueVoteEnact(controller.LeagueId, entry.ResolutionId, controller.ActiveiPlayer, votes, choice);	
				else
					Network.SendLeagueVoteRepeal(controller.LeagueId, entry.ResolutionId, controller.ActiveiPlayer, votes, choice);	
				end
			end
		end
		
		if (controller.VotesAvailable > 0) then
			Network.SendLeagueVoteAbstain(controller.LeagueId, controller.ActiveiPlayer, controller.VotesAvailable);
		end
	end
...
function VoteMajorCivController:UpdateVoteInstance()
	...
	if(choice ~= nil) then
		...
		for i,v in ipairs(self.Entry.VoterChoices) do
			if(v.Id == choice) then
				mainIcon = v.MainIcon;
				choiceiPlayer = v.iPlayer;
			end
		end
		
		IconHookup(mainIcon.PortraitIndex, 64, mainIcon.IconAtlas, instance.AltIcon);
		CivIconHookup(choiceiPlayer, 32, instance.AltSubIcon, instance.AltSubIconBG, instance.AltSubIconShadow, true, true);
	end
...
function ProposalController:UpdatePendingProposalInstance(pendingProposal)
		...
		PopulateProposeResolutionPopup(self.ActiveResolutions, self.InactiveResolutions, self.PendingProposals, function(proposal)
			...
			local SelectProposal = function(choiceId)
				...
				if(proposal.Direction == "Enact") then
					...
					toolTip = league:GetResolutionDetails(proposal.Type, controller.ActiveiPlayer, -1, choiceId); 				
				else
					prefix = g_strRepealPrefix;
					text = league:GetResolutionName(proposal.Type, -1, proposal.ProposerDecision, false);
					toolTip = league:GetResolutionDetails(proposal.Type, controller.ActiveiPlayer, -1, proposal.ProposerDecision); 	
				end
	...
	CivIconHookup(self.ActiveiPlayer, 32, instance.SubIcon, instance.SubIconBG, instance.SubIconShadow, true, true);
	...
	local confirmAction = function()
		for i, pendingProposal in ipairs(controller.PendingProposals) do
			...
			if(proposal.Direction == "Enact") then
				...
				Network.SendLeagueProposeEnact(controller.LeagueId, proposal.Type, controller.ActiveiPlayer, choice);
			else
				Network.SendLeagueProposeRepeal(controller.LeagueId, proposal.Id, controller.ActiveiPlayer);
			end
		end	
	end
